...
(function () {
   ...
   * @param {Object} [options] - 可选，{ pageType, categoryTemplate, mineTemplate, pageName }
   */
  function openEditor(id, options) {
    ...
    } else if (id === 'new' && options && options.pageType === 'mine' && options.mineTemplate) {
      name = options.pageName || ('新建我的页面 - ' + (MINE_TEMPLATE_NAMES[options.mineTemplate] || options.mineTemplate));
    } else {
    ...
    if (options && options.mineTemplate) {
      url.searchParams.set('mineTemplate', options.mineTemplate);
    }
   ...
   * 打开「我的页面选择模板」弹窗
   */
  function openMineTemplateModal() {
    const modal = document.getElementById('modal-mine-template');
    if (!modal) return;
    const first = document.querySelector('#mine-template-list input[name="mine-template"]');
    if (first) first.checked = true;
    modal.classList.add('show');
  }
   ...
   * 确认所选我的页面模板并进入编辑器
   */
  function confirmMineTemplate() {
    const radio = document.querySelector('#modal-mine-template input[name="mine-template"]:checked');
    if (!radio) {
      alert('请先选择一种我的页面模板。');
      return;
    }
    ...
    if (modal) modal.classList.remove('show');
    const value = radio.value;
    const pageName = '新建我的页面 - ' + (MINE_TEMPLATE_NAMES[value] || value);
    openEditor('new', { pageType: 'mine', mineTemplate: value, pageName });
  }
  ...
  document.getElementById('btn-confirm-mine-template')?.addEventListener('click', confirmMineTemplate);

  // 页面管理 - 新建页面（首页/海报/文章直接进编辑器，分类页/我的页面先选模板）
  document.getElementById('btn-new-page')?.addEventListener('click', function () {
    ...
    } else if (currentTemplatePageType === 'mine') {
      openMineTemplateModal();
    } else {
      openEditor('new');
    }
  });
