...
| 目标分类 | targetCategory | 下拉选择 | 是 | String | - | 必须选择 | 已有一级或二级分类 | 系统数据 | 请选择目标分类 | 用于移动素材 |
| 上传文件 | uploadFile | 文件上传 | 是 | File | - | 格式、大小校验 | 见业务规则 | 用户选择 | 请选择要上传的文件 | 支持批量上传 |
| 搜索关键词 | searchKeyword | 文本输入 | 否 | String | - | - | - | 用户输入 | 搜索素材名称 | 模糊匹配 |
...
### 4.1.2. 页面装修模块【模块标题下无内容】

#### 4.1.2.1. 配置面板页面【除必须外，其余按需决定是否配置】

##### 1. 功能概述【必须】
- **功能描述**：配置面板用于页面装修时选择图片或视频素材。通过选择图片弹窗和选择视频弹窗，从素材库中选择需要的素材。
- **业务描述**：运营人员在页面装修场景下，需要配置图片或视频组件时，通过配置面板的素材选择功能从素材库中选取素材。
- **使用角色**：运营人员
- **触发条件**：页面装修 → 配置图片/视频组件 → 点击选择素材

##### 2. 页面原型【必须，可留白后续补充】
- 选择图片弹窗：展示图片库素材列表，支持分类筛选和搜索
- 选择视频弹窗：展示视频库素材列表，支持分类筛选和搜索

##### 3. 输入项说明

| 字段名称 | 字段标识 | 字段类型 | 必填 | 数据类型 | 长度限制 | 校验规则 | 取值范围 | 来源 | 提示文案 | 备注 |
|----------|----------|----------|------|----------|----------|----------|----------|------|----------|------|
| 素材类型 | materialType | 单选 | 是 | String | - | - | image/video | 系统配置 | - | 根据组件类型自动确定 |
| 分类筛选 | categoryFilter | 下拉选择 | 否 | String | - | - | 素材库分类 | 系统数据 | 请选择分类 | - |
| 搜索关键词 | searchKeyword | 文本输入 | 否 | String | - | - | - | 用户输入 | 搜索素材名称 | 模糊匹配 |
...
| 字段名称 | 字段标识 | 数据类型 | 数据来源 | 格式说明 |
|----------|----------|----------|----------|----------|
| 选中素材 | selectedMaterial | MaterialItem | 用户选择 | 包含id、name、url等 |
| 素材URL | materialUrl | String | 选中素材 | 用于组件渲染 |

##### 5. 业务规则

| 规则编号 | 规则名称 | 适用场景 | 规则描述 | 错误提示 |
|----------|----------|----------|----------|----------|
| BR-601 | 素材类型匹配 | 选择素材 | 只能选择与组件类型匹配的素材 | 请选择图片/视频素材 |
...
| 版本号 | 修改日期 | 修改人 | 修改内容 |
|--------|----------|--------|----------|
| V1.1 | 2026-03-20 | 产品经理 | 重新生成，添加五级标题序号 |